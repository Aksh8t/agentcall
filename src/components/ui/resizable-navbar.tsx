...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  useMotionValueEvent(scrollY, "change", (latest) => {
    ...
    // Determine if navbar should be visible (styled)
    if (latest > 100) {
      setVisible(true);
    } else {
      setVisible(false);
    ...
    // Determine if navbar should be hidden (floating behavior)
    if (latest > 150) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        visible && "bg-white/95 dark:bg-neutral-950/95 backdrop-blur-xl border border-white/20 shadow-[0_8px_32px_0_rgba(31,38,135,0.37)]",
        className,
      )}
    >
      {React.Children.map(children, (child) =>
        ...
        >
          {item.link ? (
            ...
            <motion.p
              ...
              className="relative cursor-pointer px-4 py-2 text-neutral-600 hover:opacity-[0.9] dark:text-neutral-300"
            >
              <span className="relative z-20">{item.name}</span>
            </motion.p>
          )}
          {active !== null && active === item.name && item.children && (
            <motion.div
...
export const MobileNav = ({ children, className, visible }: MobileNavProps) => {
  return (
    ...
    >
      {React.Children.map(children, (child) =>
        React.isValidElement(child) && (child.type === MobileNavHeader)
          ? React.cloneElement(
              child as React.ReactElement<{ visible?: boolean }>,
              { visible },
            )
